...
(function($){
	$.fn.mailAutoComplete = function(options){
		var defaults = {
			...
			newReg: /qq\.com|gmail\.com|126\.com|163\.com|hotmail\.com|yahoo\.com|yahoo\.com\.cn|live\.com|sohu\.com|sina\.com/,
		...
		var z = settings.zIndex, newArr = mailArr = settings.mailArr, hint = settings.textHint, text = settings.hintText, fc = settings.focusColor, bc = settings.blurColor,newReg = settings.newReg;
		//创建邮件内部列表内容
		...
		$(this).each(function(){
			...
			that.wrap('<span style="display:inline-block;position:relative;"></span>')
				.before('<ul id="mailListBox_'+i+'" class="justForJs '+cb+'" style="min-width:'+w+'px;_width:'+w+'px;position:absolute;left:-6000px;top:'+h+'px;z-index:'+z+';"></ul>');
			...
			that.focus(function(){
				...
				$(this).keyup(function(e){
					...
					if(v.length > 0){
						...
						var emailList = $.createHtml(s, newArr, index);
						if (emailList != ''){
							x.html(emailList).css("left", 0);
						}else{
							x.css("left", "-6000px");	
						}
